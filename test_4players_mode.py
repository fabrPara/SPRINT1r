...
class Test4PlayersMode:
    ...
    def test_set_game_mode_4_players(self):
        """Test che QuoridorGame(num_players=4) crea 4 giocatori."""
        ...
        assert game._players[0].get_position().get_coords() == (1, 5)  # P1
        assert game._players[1].get_position().get_coords() == (9, 5)  # P2
        assert game._players[2].get_position().get_coords() == (5, 1)  # P3
        assert game._players[3].get_position().get_coords() == (5, 9)  # P4
    ...
    def test_target_rows_4_players(self):
        ...
        # P1 e P2 hanno target row negativo (colonna)
        assert game._players[0]._target_row == -9  # colonna 9
        assert game._players[1]._target_row == -1  # colonna 1

        # P3 e P4 hanno target row positivo (riga)
        assert game._players[2]._target_row == 9  # riga 9
        assert game._players[3]._target_row == 1  # riga 1
    ...
    def test_check_victory_p1_p2_row_target(self):
        """Test che P1 e P2 vincono raggiungendo le colonne target."""
        game = QuoridorGame(num_players=4)

        # P1 raggiunge colonna 9
    ...
    def test_check_victory_p3_p4_col_target(self):
        """Test che P3 e P4 vincono raggiungendo le righe target."""
        game = QuoridorGame(num_players=4)

        # Resetta e testa P3 (target riga 9)
        game._winner = None
    ...
    def test_resign_4_players_removes_from_cycle(self):
        ...
        assert winner == 0  # Nessun vincitore, partita continua
        assert game._current_turn == 2
    ...
    def test_resign_4_players_last_one_wins(self):
        ...
        # Rimuovi 3 giocatori
        game._active_player_ids = [1, 2, 3, 4]
    ...
    def test_get_game_state_includes_4_player_info(self):
        ...
        assert state["active_player_ids"] == [1, 2, 3, 4]
        assert len(state["players"]) == 4

    def test_reset_returns_to_2_players(self):
        """Test che un nuovo gioco di default sia a 2 giocatori."""
    ...
    def test_invalid_game_mode(self):
        ...
        # QuoridorGame non valida num_players nel costruttore,
        # il controllo avviene a livello applicativo
        try:
